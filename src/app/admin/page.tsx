...
export default function AdminPage() {
  const [keys, setKeys] = useState<{ value: string; activated: boolean }[]>([]);
  const [filter, setFilter] = useState<"all" | "activated" | "notActivated">("all");
  const [keyCount, setKeyCount] = useState(1);
  ...
  const saveKeyManually = async () => {
    ...
    try {
      await fetch("/api/admin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ keys: [manualKey] }),
      });
      ...
      console.error("Failed to save manual key:", err);
    }
  };
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <div className="border-t pt-4 mt-4">
            <h3 className="text-lg font-semibold">Add Key Manually</h3>
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 mt-2">
              ...
              />
              <Button onClick={saveKeyManually}>Save Key</Button>
            </div>
          </div>
